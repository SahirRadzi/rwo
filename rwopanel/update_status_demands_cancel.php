...
<head>
   ...
   <title>Update Status Demands Cancel | RWO Panel</title>
...
<section class="placed-orders">
   ...
   <h1 class="heading">update status cancel | <?= $jumlah_permintaanBaru_batal;?></h1>
...
if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
   ...
   $select_listings = $conn->prepare("SELECT demand_list.id, demand_list.unique_id, demand_list.tarikh, demand_list.nama, demand_list.email, demand_list.phoneno, demand_list.nokp, demand_list.alamatPemasangan, demand_list.alamatBill, products.s_name, products.price, demand_list.tarikhWaktu, demand_list.signa_c, demand_list.imgBill, demand_list.imgKpD, demand_list.imgKpB, demand_list.status FROM demand_list INNER JOIN products ON demand_list.pid = products.id WHERE status = 'Batal' AND unique_id LIKE '%{$search_box}%' AND nokp LIKE '%{$search_box}%'");
   $select_listings->execute();
}else

$select_listings = $conn->prepare("SELECT demand_list.id, demand_list.unique_id, demand_list.tarikh, demand_list.nama, demand_list.email, demand_list.phoneno, demand_list.nokp, demand_list.alamatPemasangan, demand_list.alamatBill, products.s_name, products.price, demand_list.tarikhWaktu, demand_list.signa_c, demand_list.imgBill, demand_list.imgKpD, demand_list.imgKpB, demand_list.status FROM demand_list INNER JOIN products ON demand_list.pid = products.id WHERE status = 'Batal'");
$select_listings->execute();
...
<div class="box-container">
   ...
   <?php
      ...
      echo '<p class="empty">no demands cancel yet!</p>';
   }
   ?>
   </div>
